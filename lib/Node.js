  ...
  async.series([
      ...
      function(c) {
        ...
        txList.forEach(function(tx) {
          self._storeTx(tx, false, function(err) {
            c(err);
          });
        });
      }
...
Node.prototype._storeTx(tx, mempool, cb) {
  var self = this;
  var oldTx = undefined;
  async.series([
      function(c) {
...
Node.prototype._updateTxSpent = function(tx, archive, cb) {
  var self = this;
  tx.vin.forEach(function(input, idx) {
    var inputTx = undefined;
    async.series(
      function(c) {
        self.store.getTx(input.hash, function(err, tx) {
          if(err) return c(err);
          if(!tx) return c(
              new Skip('tx:'+tx.hash.toString('hex')+'\'s input:'
                +input.hash.toString('hex')+' not exist'));
          inputTx = tx;
          c();
        });
      },
      function(c) {
        var spent = {};
        if(archive) {
            spent['vout.'+input.n+'.spent'] = inputTx.vout[input.n].spent-1;
        } else {
            spent['vout.'+input.n+'.spent'] = inputTx.vout[input.n].spent+1;
        }
        self.store.updatePartialTx(inputTx.hash, spent, function(err) {
          if(err) return c(err);
          c();
        });
      },
      function(c) {
        if(archive) return c();
        var up = {};
        up['vin.'+idx+'.addrs'] = inputTx.vout[input.n].addrs;
        up['vin.'+idx+'.v'] = inputTx.vout[input.n].v;
        self.store.updatePartialTx(tx.hash, up, function(err) {
          if(err) return c(err);
          c();
        });
      }],
