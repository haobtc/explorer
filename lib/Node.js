...
Node.prototype.handleTx = function(info) {
  ...
  if(this.updateMempool) {
    ...
    self._checkTx(txObj, function(err, ret) {
      if(err) throw err;
      if(!ret) return;
...
Node.prototype.sendTx = function(tx, cb) {
  ...
  self._checkTx(txObj, function(err, ret) {
    ...
    if(!ret) return cb(undefined, {code:-6, message:'transaction illegal.'});
    self._addTx(tx, true, function(err, exist) {
      if(err) return cb(err, false);
...
Node.prototype._checkTx = function(tx, cb) {
  var self = this;
  if(tx.vin.length === 0) return cb(undefined, true);
  var tasks = tx.vin.map(function(input) {
    return function(c) {
      if(!input.hash) return c();
      self.store.getTx(input.hash, function(err, val) {
        if(err) return c(err);
        if(!val) return c(new Skip('tx='+tx.hash.toString('hex')+
            '\'s input hash='+input.hash.toString('hex') + 'not found'));
        c();
      });
    };
  });
...
Node.prototype._onAddrPropChanged = function(tx, del, cb) {
  var self = this;
  this.store.getTx(tx.hash, function(err, val) {
    if(err) return cb(err);
    if(!val) {
      console.error('_onAddrPropChanged:tx not exist:hash=%s',
    ...
    var inList = [];
    var outList = [];
    newTx.vin.forEach(function(input) {
      if(!input.hash) return;
      input.addrs.forEach(function(addr) {
        var tmp = {};
        tmp['addr'] = addr;
        tmp['v'] = input.v;
        inList.push(tmp);
    ...
    newTx.vout.forEach(function(output) {
      if(!output.s) return;
    ...
    var tasks = inList.map(function(e) {
      return function(c) {
        var v = Number(e.v);
        if(del) v = -v;
        self.store.updateAddrProp(e.addr, {totalSent:v}, function(err) {
          if(err) return c(err);
          c();
        });
      };
    });
    tasks = tasks.concat(outList.map(function(e) {
      return function(c) {
        var v = Number(e.v);
        if(del) v = -v;
        self.store.updateAddrProp(e.addr, {totalReceived:v}, function(err) {
          if(err) return c(err);
          c();
        });
      };
