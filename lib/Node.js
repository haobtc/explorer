...
function Node(netname) {
  ...
  this.blockQueue = new Queue({timeout:30000, limit:510});
...
Node.prototype.handleBlock = function(info) {
  ...
  this.store.getBlock(blockObj.hash, function(err, b) {
    if(err) throw err;
    if(!!b) {
      return;
    }
    ...
    self.blockQueue.task(function(c) {
...
Node.prototype.processMempool = function(tx, callback) {
  ...
  var lastID = new mongodb.ObjectID.createFromTime(Math.floor(new Date().getTime()/1000 - 86400 * 7));
  ...
  async.series([
	...
	console.info('send mempool tx', inv.hash.toString('hex'), 'to', conns.length, 'nodes');
      });
      conns.forEach(function(conn) {
	if(Math.random() < 0.5) {
