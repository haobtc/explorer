...
Node.prototype.requireBlocks = function() {
  ...
  this.store.getTipBlock(function(err, tipBlock) {
	...
	self.store.getNthPrevBlock(tipBlock, 6, function(err, b) {
	  if(err) throw err;
	  if(b) {
	    var conn2 = self.randomConn();
	    if(!!conn2) {
	      var pHash = helper.reverseBuffer(b.hash);;
	      conn2.sendGetBlocks([pHash], 0);
	      console.info(self.netname, 'getting blocks starting from prev5 ', pHash.toString('hex'), 'from', conn2.peer.host);
	    }
	  }
	});
      }
