...
Node.prototype.archiveTxes = function(blockHashList, cb) {
  ...
  _archiveAction(blockHashList, true, cb);
};

Node.prototype.unArchiveTxes = function(blockHashList, cb) {
  console.info('unArchiveTxes', blockHashList);
  _archiveAction(blockHashList, false, cb);
};

Node.prototype._archiveAction = function(blockHashList, archive, cb) {
  if(!blockHashList || blockHashList.length == 0) return cb();
  var self = this;
  this.store.getTxListByBlock(blockHashList, function(err, vals) {
    if(err) return cb(err);
    if(vals.length <= 0) return cb();
    var tasks = vals.map(function(tx) {
      return function(c) {
        var del = false;
        if(archive) del = true;
        self._onTxChanged(tx, del, function(err) {
          if(err) return c(err);
          c();
        });
      };
...
Node.prototype.handleTx = function(info) {
  ...
  if(this.updateMempool) {
    ...
    self._addTx(txObj, true, function(err, exist) {
      if(err) throw err;
      if(exist) return;
      self._onTxChanged(txObj, false, function(err) {
        if(err) throw err;
      });
    });
  }
      ...
      if(tx && tx.raw) {
        conns.forEach(function(conn) {
          console.info('send pending tx:netname=%s:txhash=%s:peerHost=%s',
            self.netname, tx.hash.buffer.toString('hex'), conn.peer.host);
...
Node.prototype.processMempool = function(tx, callback) {
  ...
  this.store.removeObsoleteTxList(function(err, txList) {
    if(err) throw err;
    var tasks = txList.map(function(tx) {
      return function(c) {
        self._onTxChanged(tx, true, function(err) {
          if(err) return c(err);
          c();
        });
      };
...
Node.prototype.addNewBlock = function(b, cb) {
  ...
  this.store.insertBlock(b, function(err, txes) {
    if(err) return cb(err);
    console.log('addNewBlock:txes length=%d', txes.length);
    var newTxDict = {};
    var tasks = txes.map(function(tx) {
      return function(c) {
        self._addTx(tx, false, function(err, exist) {
          if(err) return c(err);
          if(!exist) newTxDict[tx.hash.toString('hex')] = true;
          c();
        });
      };
    });
    tasks = tasks.concat(txes.map(function(tx) {
      return function(c) {
        if(!newTxDict[tx.hash.toString('hex')]) return c();
        self._onTxChanged(tx, false, function(err) {
          if(err) return c(err);
          c();
        });
      };
...
Node.prototype._addTx = function(tx, mempool, cb) {
  console.log('_addTx:hash=%s:bhash=%s:mempool=%d',
      tx.hash.toString('hex'), tx.bhash.toString('hex'), mempool);
  var self = this;
  var oldTx = undefined;
  var binfo = [];
  async.series([
      ...
      function(c) {
        if(mempool) {
          if(oldTx) return c(new Skip('_addTx:tx exist:hash='+tx.hash.toString('hex')
              +':mempool=' + mempool));
          else return c();
        }
        if(!oldTx) {
          tx.binfo = [{bhash:tx.bhash,bidx:tx.bidx}];
          delete tx.bhash;
          delete tx.bidx;
          return c();
        }
      ...
      function(err) {
        if(err && err instanceof Skip) err = null;
        return cb(err, oldTx !== undefined);
      });
};

Node.prototype._onTxChanged = function(tx, del, cb) {
  console.log('_onTxChanged:hash=%s:del=%d', tx.hash.toString('hex'), del);
  var self = this;
  var spentChangeList = [];
  var receiveChangeList = [];
  // tx changes vin tx's spent
  var tasks = tx.vin.map(function(input, i) {
    return function(c) {
      if(!input.hash) return c();
      console.log('_onTxChanged:inputHash=%s', input.hash.toString('hex'));
      var inputTx = undefined;
      async.series([
        function(cc) {
          self.store.getTx(input.hash, function(err, val) {
            if(err) return cc(err);
            if(!val) {
              console.error('txHash inputHash not exist:txHash=%s:inputHash=%s',
                tx.hash.toString('hex'), input.hash.toString('hex'));
              return cc(new Skip('txHash=%s'+tx.hash.toString('hex')
                + ':inputHash not found'));
            }
            inputTx = val;
            cc();
        ...
        function(cc) {
          var spentInfo = {};
          spentInfo['vout.'+input.n+'.spent'] = del ? -1 : 1;
          self.store.updateSpent(inputTx.hash, spentInfo, function(err) {
            if(err) return cc(err);
            cc();
          });
        },
        function(cc) {
          if(del) return cc();
          var vinInfo = {};
          vinInfo['vin.'+i+'.addrs'] = inputTx.vout[input.n].addrs;
          vinInfo['vin.'+i+'.v'] = inputTx.vout[input.n].v;
          self.store.updatePartialTx(tx.hash, vinInfo, function(err) {
            console.log('updatePartialTx:txHash=%s', tx.hash.toString('hex'));
            if(err) return cc(err);
            cc();
          });
        },
        function(cc) {
          var addrs = inputTx.vout[input.n].addrs;
          var val = inputTx.vout[input.n].v;
          addrs.forEach(function(addr) {
            var spentChangeInfo = {};
            spentChangeInfo.th = inputTx.hash;
            spentChangeInfo.a = addr;
            spentChangeInfo.n = input.n;
            spentChangeInfo.v = val;
            spentChangeInfo.d = del ? -1 : 1;
            spentChangeList.push(spentChangeInfo);
          });
          cc();
        }
        ],
  ...
  // tx changes self spent
  tasks = tasks.concat(tx.vout.map(function(output, i) {
    return function(c) {
      var txList = undefined;
      async.series([
        function(cc) {
          output.addrs.forEach(function(addr) {
            var receiveChangeInfo = {};
            receiveChangeInfo.a = addr;
            receiveChangeInfo.v = output.v;
            receiveChangeList.push(receiveChangeInfo);
          });
          var key = tx.hash.toString('hex')+'#'+i;
          console.log('_onTxChanged:key=%s', key);
          self.store.getTxListByVout(key, function(err, txes) {
            if(err) return cc(err);
            if(!txes || txes.length === 0) {
              return cc(new Skip('key\'s txes empty'));
            }
            txList = txes;
            cc();
          });
        ...
        function(cc) {
          var spentInfo = {};
          spentInfo['vout.'+i+'.spent'] = del ? -txList.length: txList.length;
          self.store.updateSpent(tx.hash, spentInfo, function(err) {
            if(err) return cc(err);
            cc();
          });
        },
        function(cc) {
          var updateAddrTasks = txList.map(function(e) {
            return function(c) {
              var vinInfo = {};
              vinInfo['vin.'+i+'.addrs'] = output.addrs;
              vinInfo['vin.'+i+'.v'] = output.v;
              self.store.updatePartialTx(e.hash, vinInfo, function(err) {
                if(err) return c(err);
                c();
              });
            };
          });
          async.series(updateAddrTasks, function(err) {
            if(err) return cc(err);
            cc();
          });
        },
        function(cc) {
          output.addrs.forEach(function(addr) {
            var spentChangeInfo = {};
            spentChangeInfo.th = tx.hash;
            spentChangeInfo.a = addr;
            spentChangeInfo.n = i;
            spentChangeInfo.d = del ? -txList.length: txList.length;
            spentChangeInfo.v = output.v;
            spentChangeList.push(spentChangeInfo);
          });
          cc();
        }
  ...
  async.series(tasks, function(err) {
    ...
    if(err) return cb(err);
    self._onAddrPropChanged(spentChangeList, receiveChangeList, function(err) {
      if(err) return cb(err);
      cb(undefined);
    });
  });
...
Node.prototype._onAddrPropChanged = function(spentChangeList, receiveChangeList, cb) {
  ...
  var tasks = spentChangeList.map(function(spentChangeInfo) {
    return function(c) {
      // spentChangeInfo, 0->positive: balance decrease, positive->0: balance increase
      // {th:"", a:"", n:"", d:""}
      async.series([
        function(cc) {
          //get change flag: 0 indicates balance decrease, otherwise 1
          self.store.getTx(spentChangeInfo.th, function(err, tx) {
            if(err) return c(err);
            if(!tx) return c(new Skip(''));
            var cur = tx.vout[spentChangeInfo.n].spent;
            var original = cur-spentChangeInfo.d;
            if(original === 0 && cur > 0) spentChangeInfo.v = -(Number(spentChangeInfo.v));
            else if(original > 0 && cur === 0) spentChangeInfo.v = Number(spentChangeInfo.v);
            else return cc(new Skip(''));
            cc();
          });
        },
        function(cc) {
  ...
  // addr's total receive change
  tasks = tasks.concat(receiveChangeList.map(function(receiveChangeInfo) {
    return function(c) {
      // receiveChangeInfo: change addr's total received value
      // {a:"",v:""}
      var updateInfo = {totalReceived:Number(receiveChangeInfo.v), balance:Number(receiveChangeInfo.v)};
