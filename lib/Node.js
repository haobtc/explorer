...
Node.prototype.addTxes = function(txes, callback) {
  var self = this;
  var txcol = this.store.dbconn.collection('tx');
  var newTxes = [];
  ...
  async.series(txes.map(insertTx),
	       function(err) {
		 if(err) return callback(err);
		 console.info('new txes', newTxes.length);
...
Node.prototype.handleTx = function(info) {
  ...
  if(this.updateMempool) {
    ...
    this.addTxes([txObj], function(err) {
      if(err) throw err;
    });
  }
};
...
Node.prototype.incSpent = function(txes) {
...
Node.prototype.decSpent = function(txes) {
  var col = this.store.dbconn.collection('tx');
  txes.forEach(function(tx) {
    tx.vin.forEach(function(input) {
      if(input.hash && input.n >= 0) {
