...
Node.prototype.handleTx = function(info) {
  ...
  if(this.updateMempool) {
    ...
    this._storeTx(txObj, true, function(err) {
...
Node.prototype.sendTx = function(tx, cb) {
  ...
  this._storeTx(txObj, true, function(err, exist) {
...
Node.prototype.processMempool = function(tx, callback) {
  ...
  this.store.removeObsoleteTx(function(err) {
    if(err) throw err;
  });
...
Node.prototype.addNewBlock = function(b, cb) {
  ...
  this.store.insertBlock(b, function(err, txes) {
    ...
    var tasks = txes.map(function(tx) {
      return function(c) {
        self._addTx(tx, false, function(err) {
          if(err) return c(err);
          c();
        });
      };
    ...
    tasks = tasks.concat(txes.map(function(tx) {
      return function(c) {
        self._updateTx(tx, false, function(err) {
          if(err) return c(err);
          c();
        });
      };
...
Node.prototype._updateTx = function(tx, del, cb) {
  console.log('_updateTx:hash=%s:del=%d', tx.hash.toString('hex'), del);
  var self = this;
  var tasks = tx.vin.map(function(input, i) {
    return function(c) {
      if(!input.hash) return c();
      console.log('_updateTx:inputHash=%s', input.hash.toString('hex'));
      ...
      async.series([
        ...
        function(cc) {
          ...
          var spentInfo = {};
          if(del) {
            spentInfo['vout.'+input.n+'.spent'] = inputTx.vout[input.n].spent-1;
          } else {
            spentInfo['vout.'+input.n+'.spent'] = inputTx.vout[input.n].spent+1;
          }
          self.store.updatePartialTx(inputTx.hash, spentInfo, function(err) {
            if(err) return cc(err);
            cc();
          });
        },
