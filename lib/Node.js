...
Node.prototype._onTxChanged = function(tx, del, cb) {
  ...
  async.series(tasks, function(err) {
    ...
    if(err) return cb(err);
    self._onAddrPropChanged(tx, del, function(err) {
      if(err) return cb(err);
      cb(undefined);
    });
  });
...
Node.prototype._onAddrPropChanged = function(spentchangeList, receiveChangeList, cb) {
      ...
      async.series([
        function(cc) {
          ...
          self.store.getTx(spentChangeInfo.th, function(err, tx) {
            ...
            spentChangeInfo[v] = tx.vout[spentChangeInfo.n].v;
            if(original === 0 && cur > 0) spentChangeInfo[f] = 0;
            else if(original > 0 && cur === 0) spentChangeInfo[f]=1;
            else return cc(new Skip(''));
            cc();
          });
        },
        function(cc) {
          self.store.updateAddrProp(spentChangeInfo.a, {balance:Number(spentChangeInfo.v)}, function(err) {
            if(err) return cc(err);
            cc();
          });
        }
  ...
  tasks = tasks.concat(receiveChangeList.map(function(receiveChangeInfo) {
    return function(c) {
      ...
      // {a:"",v:""}
      self.store.updateAddrProp(receiveChangeInfo.a, {totalReceived:Number(receiveChangeInfo.v)}, function(err) {
        if(err) return c(err);
        c();
      });
    };
