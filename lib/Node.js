...
Node.prototype.addNewBlock = function(b, cb) {
  ...
  this.store.insertBlock(b, function(err, txes) {
    ...
    /*var tasks = txes.map(function(tx) {
      return function(c) {
        self._addTx(tx, false, function(err, exist) {
          if(err) return c(err);
          if(exist) return c();
          console.log('addTx:hash=%s:exist=%d', tx.hash.toString('hex'), exist);
    ...
    */
   txes.forEach(function(tx) {
     self._addTx(false, function(err, exist) {
       if(err) return cb(err);
     });
   });
  });
...
Node.prototype._addTx = function(tx, mempool, cb) {
  ...
  this.store.insertTx2(tx, function(err, oldTx) {
    if(err) return cb(err);
    //if(oldTx && !oldTx.binfo && !mempool) return cb(undefined, true);
...
Node.prototype._onTxChanged = function(tx, del, cb) {
  var self = this;
  //console.log('_onTxChanged:txHash=%s:del=%d', tx.hash.toString('hex'), del);
  ...
  tx.vin.forEach(function(input, i) {
    ...
    otherVoutTasks.push(function(c) {
      ...
      //console.log('_onTxChanged:txHash=%s:inputHash=%s:del=%d', tx.hash.toString('hex'), input.hash.toString('hex'),del);
      self.store.updateSpent(input.hash, spentInfo, function(err, inputTx) {
        if(err) return c(err);
        if(inputTx) {
          var addrs = inputTx.vout[input.n].addrs;
          var val = inputTx.vout[input.n].v;
          addrs.forEach(function(addr) {
            spentChangeList.push({
              th:inputTx.hash,a:addr,
              o:inputTx.vout[input.n].spent,
              d:del?-1:1,
              v:Number(val),d:(del?-1:1)
            });
          });
          return c();
        }
        console.log('_onTxChanged:inputHash not found:txHash=%s:inputHash=%s', tx.hash.toString('hex'), input.hash.toString('hex'));
        c();
      });
    });
  });
  tx.vout.forEach(function(output, i) {
    selfVoutTasks.push(function(c) {
      output.addrs.forEach(function(addr) {
        receiveChangeList.push({a:addr,v:(del?-Number(output.v):Number(output.v))});
      });
      var key = tx.hash.toString('hex')+'#'+i;
      self.store.getTxListByVout(key, function(err, vals) {
        if(!vals || vals.length === 0) return c();
        var spentInfo = {};
        spentInfo['vout.'+i+'.spent'] = del?-vals.length:vals.length;
        //console.log('_onTxChanged:update vout spent:key=%s:spent=%d', key, vals.length);
        self.store.updateSpent(tx.hash, spentInfo, function(err, oldTx) {
          if(err) return c(err);
          output.addrs.forEach(function(addr) {
            spentChangeList.push({
              th:tx.hash,
              a:addr,
              o:oldTx.vout[i].spent,
              d:(del?-vals.length:vals.length),
              v:Number(output.v)});
          });
          c();
        });
...
Node.prototype._onAddrPropChanged = function(spentChangeList, receiveChangeList, cb) {
  ...
  spentChangeList.forEach(function(spentChangeInfo) {
    ...
    // {th:"", a:"", n:"", o:"", d""}
    var balanceChg = 0, cur = spentChangeInfo.o + spentChangeInfo.d;
    if(spentChangeInfo.o === 0 && cur > 0) balanceChg = -spentChangeInfo.v;
    else if(spentChangeInfo.o > 0 && cur === 0) balanceChg = spentChangeInfo.v;
    if(balanceChg !== 0) {
      self.store.updateAddrProp(spentChangeInfo.a, {balance:balanceChg}, function(err) {
        if(err) cb(err);
      });
    }
  });
