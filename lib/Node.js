...
function Node(netname) {
  ...
  this.blockQueue = new Queue({timeout:30000, limit:1810});
...
Node.prototype.processTx = function(tx, blockObj) {
  return helper.processTx(this.netname, tx, blockObj);
};
...
Node.prototype.onBlock = function(block, cb) {
  var self = this;
  var pendingFunc = this.pendingBlockHashes[block.calcHash().toString('hex')];
  if(!pendingFunc) {
    if(!this.allowOldBlock &&
       this.store.tip_timestamp >= 0 &&
       block.timestamp < this.store.tip_timestamp - 86400 * 3) {
      //console.info('deny old block', this.netname, 'at', new Date(block.timestamp * 1000).toString());
      return cb(null, false);
    }
    if(!this.allowOldBlock &&
       this.store.tip_timestamp >= 0 &&
       block.timestamp > this.store.tip_timestamp + 86400 * 7) {
  ...
  if(pendingFunc) {
    ...
    delete this.pendingBlockHashes[block.calcHash().toString('hex')];
  }
  if(this.addBlocks) {
...
Node.prototype.requireBlocksFromFile = function() {
  ...
  this.store.getTipBlock(function(err, tipBlock) {
	...
	if(tipBlock.prev_hash && Math.random() < 0.1) {
      ...
      var genesisBlock = helper.clone(bitcore.networks[self.netname].genesisBlock);
      console.info(self.netname, 'getting blocks starting from', genesisBlock.hash.toString('hex'), 'to');
...
Node.prototype.requireBlocksFromNet = function() {
  ...
  this.store.getTipBlock(function(err, tipBlock) {
	...
	if(!!conn1) {
      ...
      var genesisBlock = helper.clone(bitcore.networks[self.netname].genesisBlock);
      console.info(self.netname, 'getting blocks starting from', genesisBlock.hash.toString('hex'), 'to', conn.peer.host);
