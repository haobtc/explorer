  ...
  async.series([
   ...
   function(err) {
     if(sumtime > 10.0) {
       ...
       console.info('storetipblock using:hash=%s:skip=%d', b.hash.toString('hex'), skip, tss);
     }
...
Node.prototype.onBlock = function(block, cb) {
  ...
  if(this.blockCacheSize >= 10000) {
    this.blockCacheDict = {};
    this.blockCacheSize = 0;
  }
  if(this.blockCacheDict[bhash.toString('hex')]) {
    return cb(null, false);;
  }
  this.blockCacheDict[bhash.toString('hex')] = true;
  this.blockCacheSize++;
				...
				self.storeTipBlock(blockObj, true, function(err) {
				  if(err) {
				    console.error(err);
                                  ...
                                  console.log('storeTipBlock:hash=%s:cost=%d',
                                              blockObj.hash.toString('hex'), (new Date() -s)/1000);
				  c();
				});
  ...
  if(this.updateMempool) {
    ...
    this.store.getTx(txObj.hash, function(err, val) {
      ...
      self._addTx(txObj, true, function(err, exist) {
        if(err) throw err;
        console.log('addTx:mempool:txHash=%s:exist=%d', txObj.toString('hex'), exist);
      });
    });
  }
};
