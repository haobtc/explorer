...
function Node(netname) {
  ...
  this.processSpentQueue = new Queue({timeout:60000, limit:1810});
...
Node.prototype.startSync = function() {
  ...
  this.processSpentTimer = setInterval(function() {
    ...
    /*
    self.processSpentQueue.task({timestamp:new Date()},
                                function(c) {
                                  self._processSpent(function(err, finish) {
                                    if(err) console.error(err);
                                    self.processingSpent = false;
...
Node.prototype.processMempool = function(tx, callback) {
  ...
  this.store.getObsoleteTxList(function(err, blockHashList, txList) {
    ...
    tasks.push(function(c) {
      self.store.removeObsoleteTxList(function(err) {
        if(err) return c(err);
        c();
      });
    });
    tasks.push(function(c) {
      self.store.removeObsoleteBlockList(function(err) {
        if(err) return c(err);
        c();
      });
    });
    ...
    self.store.getTxListBySinceID(val.objID, 2000, function(err, txes) {
      ...
      var tasks = txes.map(function(tx) {
        return function(c) {
          self._onTxAdd(tx, function(err) {
            if(err) return c(err);
            console.log('_onTxAdd:txHash=%s', tx.hash.toString('hex'));
            c();
          });
        };
      });
      async.series(tasks, function(err) {
        if(err) return cb(err);
        console.log('_processSpent:sinceID=%s:txHash=%s:txes.length=%d:costTime=%d',
                    val.objID, val.txHash.toString('hex'), txes.length, (new Date()-s)/1000);
