...
function getUnspentFromChain(store, addressList, callback) {
  ...
  txcol.find({'vout.addrs': {$in: addressList}}).toArray(function(err, txes) {
	...
	tx.vout.forEach(function(output, vidx) {
      ...
      //rejectSpent(store, outputs, callback);
      filterSpent(store, outputs, callback);
    }); // end of getBlocks
  });
};

var filterSpent = module.exports.filterSpent = function(store, outputs, callback) {
  if(!outputs || outputs.length == 0) {
    return callback(undefined, []);
  }
  var keys = outputs.map(function(output) { return output.key});
  store.spentDict(keys, function(err, spent) {
    if(err) return callback(err);
    outputs = underscore.filter(outputs, function(output) {
      if(!spent[output.key]) {
	delete output.key;
	return true;
      }
...
/*var rejectSpent = module.exports.rejectSpent = function(store, outputs, callback) {
  var txHashList = outputs.map(function(out) {return out.hash;});
  store.getTxDict(txHashList, function(err, dict) {
    if(err) return callback(err);
    outputs = underscore.reject(outputs, function(output) {
      var  = dict[output.hash];
      if(txInfo && txInfo.spt && txInfo.spt[output.vout]) {
	return true;
      }
      return false;
    });
...
module.exports.txListToJSON = function(store, txList, callback) {
  ...
  async.series([
	...
	if(tx._id) {
	  var idDate = tx._id.getTimestamp();
	  txObj.ts = idDate.getTime();
	  txObj.id = tx._id.toString();
	}
	...
	if(block) {
	  ...
	  txObj.confirmations = Math.max(0, store.stats.maxHeight - block.height + 1);
	} else {
	  if(tx._id) {
	    var idDate = tx._id.getTimestamp();
	    txObj.time = Math.round(idDate.getTime() / 1000.0);
	  }
...
function inputTxes(store, txList, callback) {
  ...
  txList.forEach(function(tx) {
    tx.vin.forEach(function(input) {
      if(input.hash && (!input.addrs || !input.v)) {
	inputTxHashList.push(input.hash);
      }
    });
  });
  store.getTxDictWFields(inputTxHashList, ['hash', 'vout'], callback);
};
...
function getTxListSinceID(store, since_id, colname, callback) {
  ...
  if(since_id) {
    ...
    col.find({_id: {$gt: objid}}).sort({_id: 1}).limit(50).toArray(callback);
  } else {
    col.find().sort({_id: -1}).limit(50).toArray(function(err, arr){
      if(err) return callback(err);
      arr = arr.reverse();
      callback(err, arr);
    });
...
module.exports.addRawTx = function(netname, rawtx, info, callback) {
  try {
    var txObj = decodeRawTx(netname, rawtx);
  } catch(err) {
    console.error(err);
    return callback(err);
  }
