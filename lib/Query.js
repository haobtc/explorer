...
function getTXListFromOutput(store, addressList, colname, callback) {
  var col = store.dbconn.collection(colname);
  col.find({'vout.addrs': {$in: addressList}}, ['hash', 'vout']).toArray(callback);
};

function getTXListFromInput(store, outputKeys, colname, callback) {
  var col = store.dbconn.collection(colname);
  col.find({'vin.k': {$in: outputKeys}}, ['hash']).toArray(callback);
};

module.exports.getTXList = function(addressList, callback) {
  var storeDict = getStoreDict(addressList);
  function getTXList(netname) {
	...
	sDict[a] = true;
      ...
      async.series([
	function(c) {
	  getTXListFromOutput(s.store, s.arr, 'tx', function(err, txes) {
	    if(err) return c(err);
	    txes.forEach(function(tx) {
	...
	function(c) {
	  getTXListFromOutput(s.store, s.arr, 'mempool', function(err, txes) {
	    if(err) return c(err);
	    txes.forEach(function(tx) {
	      txList.push(tx.hash.toString('hex'));
	      tx.vout.forEach(function(output, idx) {
	...
	function(c) {
	  getTXListFromInput(s.store, keys, 'tx', function(err, txes) {
	    if(err) return c(err);
	    txes.forEach(function(tx) {
	      txList.push(tx.hash.toString('hex'));
	    });
	  ...
	  getTXListFromInput(s.store, keys, 'mempool', function(err, txes) {
	    if(err) return c(err);
	    txes.forEach(function(tx) {
	      txList.push(tx.hash.toString('hex'));
	    });
