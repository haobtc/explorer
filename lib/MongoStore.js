...
Store.prototype.ensureIndices = function(callback) {
  ...
  //ensureIndex('address', {'addr': 1}, {unique: 1});
...
Store.prototype.getNthPrevBlock = function(block, n, callback) {
  var self = this;
  var pBlock = block;
  var toEnd = false;
  var i=0;
  async.whilst(function() {
    ...
    i++;
    self.getBlock(pBlock.prev_hash, function(err, b) {
      if(err) return c(err);
      if(b) {
	pBlock = b;
      } else {
    ...
    callback(err, pBlock==block?null:pBlock);
  });
};
...
Store.prototype.addTxes = function(txes, callback) {
  ...
  function insertTx(tx) {
	...
	if(!old) {
	  ...
	  old.vout.forEach(function(output, index) {
	    if(output.spt) {
	      update['vout.' + index + '.spt'] = output.spt;
	      hasChanged = true;
	    }
	  });
	  if(hasChanged) {
  ...
  async.series([
	...
	txes.forEach(function(tx) {
	  if(tx.bhs && tx.bhs.length == 1) { // Right the block.hash
		...
		new_bhs.push(tx.bhs[i]);
		new_bis.push(tx.bis[i]);
	      }
	    }
	    if(new_bhs.length == 0) {
	      removableTxes.push(tx);
	...
	console.info('changedTxes', changedTxes.length, removableTxes.length);
	c();
      });
    },
    function(c) {
...
Store.prototype.verifyTx = function(tx, callback) {
  ...
  function checkInput(tx, inputTxDict) {
	...
	if(output) {
      ...
      if(!foundInput) {
	console.err('xxxx', input.hash, 'not found');
	return false;
      }
    }
    return true;
