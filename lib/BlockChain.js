...
BlockChain.prototype.insertBlock = function(block, callback) {
  ...
  var used = new Date() - startDate;
  if(used > 1000) {
    console.info('process txes', txes.length, 'using', used/1000, 'secs');
  }
...
BlockChain.prototype.storeTipBlock = function(b, allowReorgs, cb) {
  ...
  async.series([
	...
	if(err) return c(err);
        oldTip = val;
	if(oldTip && oldTip.height >= newPrev.height + 8) {
	  return c(new Skip('Cannot insert after too old block'));
	}
...
BlockChain.prototype.setBranchOrphan = function(fromBlock, cb) {
  ...
  async.whilst(
    ...
    function(c) {
      console.warn('set orphan', pblock.hash.toString('hex'));
