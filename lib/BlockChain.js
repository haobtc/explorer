...
BlockChain.prototype.addTxes = function(txes, callback) {
  var self = this;
  var txcol = this.store.dbconn.collection('tx');
  var newTxes = [];
  ...
  async.series(txes.map(insertTx),
	       function(err) {
		 if(err) return callback(err);
...
BlockChain.prototype.storeTipBlock = function(b, allowReorgs, cb) {
  ...
  async.series([
	...
	self.store.maxHeight = b.height;
	self.store.tipTimestamp = b.timestamp;
	c();
      });
    },
    function(c) {
...
BlockChain.prototype.enqueueBlock = function(blockObj, blockContinue) {
  var self = this;
...
BlockChain.prototype.sendPendingTX = function(callback) {
  var self = this;
...
BlockChain.prototype.incSpent = function(txes) {
...
BlockChain.prototype.decSpent = function(txes) {
  var col = this.store.dbconn.collection('tx');
  txes.forEach(function(tx) {
    tx.vin.forEach(function(input) {
      if(input.hash && input.n >= 0) {
